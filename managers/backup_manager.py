...
class BackupManager:
    ...
    def list_backups(self, protocol):
        ...
        # Wrap in `sh -c` so the whole `&&` chain and the pipe run under sudo.
        # Without it, run_sudo_command elevates only the first command on
        # non-root logins and `find` runs unprivileged against a 0700 dir.
        out, err, code = self.ssh.run_sudo_command(f"sh -c {shlex.quote(cmd)}")
        if code != 0:
...
""".strip()
        ...
        # run_sudo_script (not run_sudo_command): on non-root logins the latter
        # wraps the command as `echo pass | sudo -S -p '' <cmd>`, which mangles
        # a multi-line script with a heredoc. run_sudo_script uploads it and
        # runs `sudo bash file` instead. Timeout raised from the 60s default —
        # copying config dirs plus tar can outlast it on a busy server.
